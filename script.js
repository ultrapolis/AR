...
sceneEl.addEventListener("arReady", () => { status.innerHTML = "ГОТОВО! Наведите на страницу"; });

// СТРАНИЦА 1: ВИДЕО
const t0 = document.querySelector('#target0');
t0.addEventListener("targetFound", () => {
    video.currentTime = 0; video.play();
    playBtn.style.display = 'block'; status.innerHTML = "Смотрим видео...";
});
t0.addEventListener("targetLost", () => { video.pause(); playBtn.style.display = 'none'; });

// СТРАНИЦА 2: МОДЕЛЬ
const t1 = document.querySelector('#target1');
t1.addEventListener("targetFound", () => { status.innerHTML = "Крутите модель пальцем!"; });
...
// УНИВЕРСАЛЬНОЕ ВРАЩЕНИЕ (для обеих моделей)
...
window.addEventListener('touchmove', (e) => {
    ...
    // Выбираем, какую модель крутить (какая сейчас видна)
